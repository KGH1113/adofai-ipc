...
if [ "${ADOFAIIPC_GUARDS_LOADED:-0}" = "1" ]; then return 0; fi
ADOFAIIPC_GUARDS_LOADED=1
...
assert_child_path() {
  ...
  case "$target" in "$parent"/*) ;; *) fail "Refusing to operate outside $parent: $target" ;; esac
}

safe_remove_tree() {
  assert_child_path "$1" "$2"
  rm -rf "${1%/}"
}
